...
require([
  ...
  sceneView = new SceneView({
    ...
    camera: {
      position: {
        longitude: CFG.INIT_LON,
        latitude: CFG.INIT_LAT,
        z: CFG.INIT_Z,
      },
      heading: 0,
      tilt: 0,
  ...
  function flyTo(idx) {
    ...
    const alt = ev.rank >= 88 ? 1800000 : ev.rank >= 75 ? 2800000 : 4200000;

    sceneView
      .goTo(
        {
          position: { longitude: ev.lon, latitude: ev.lat, z: alt },
          heading: 0,
          tilt: 18,
        },
    ...
    sceneView.hitTest(evt).then((res) => {
      const hit = res.results.find(
        (r) =>
          ...
          r.graphic.attributes.eventIndex != null,
      );
      if (hit) {
  ...
  function buildTicker() {
    ...
    EVENTS_DATA.forEach((ev, idx) => {
      ...
      const day =
        !de || d?.getTime() === de?.getTime() || ds === de2
          ? String(ds)
          : ds + "–" + de2;
      ...
      card.innerHTML =
        '<div class="card-bar" style="background:' +
        col +
        '"></div>' +
        '<div class="card-dt">' +
        '<span class="dt-month">' +
        ...
        "</div>" +
        '<div class="ci-attend">ATTENDANCE: ' +
  ...
  function highlightCard(idx) {
    ...
    const track = document.getElementById("tickerTrack");
    const target =
      card.offsetLeft - ticker.clientWidth / 2 + card.clientWidth / 2;
  ...
  function setTrackX(x) {
    trackTX = x;
    document.getElementById("tickerTrack").style.transform =
      "translateX(" + x + "px)";
  }
  ...
  function enableDragScroll() {
    ...
    window.addEventListener(
      ...
      (e) => {
        ...
        const nx = Math.min(
          0,
          Math.max(maxScroll(), e.touches[0].clientX - dragStartX),
        );
        trackTX = nx;
        track.style.transform = "translateX(" + nx + "px)";
      },
  ...
  function scoreFactors(ev) {
    ...
    const athStr = athCount
      ? ev.athletes.slice(0, 2).join(", ") +
        (athCount > 2 ? " +" + (athCount - 2) + " more" : "")
      : "";
    factors.push({
      ...
      rationale:
        athCount === 0
          ...
          : athStr +
            ...
            " Nike athlete" +
            (athCount > 1 ? "s" : "") +
            " guarantee on-event brand visibility",
    });
    ...
    factors.push({
      ...
      rationale:
        att >= 200000
          ? att.toLocaleString() +
            " est. attendees — stadium-scale retail activation and experiential marketing"
          : att >= 50000
            ? att.toLocaleString() +
              " est. attendees — strong in-venue brand presence and direct consumer touchpoints"
            : att.toLocaleString() +
              " est. attendees — targeted community engagement and grassroots brand building",
    });
    ...
    factors.push({
      ...
      rationale:
        daysOut > 120
          ? daysOut +
            " days out — full lead time available for complete assortment and regional distribution"
          : daysOut > 60
            ? daysOut +
              " days out — adequate window for priority SKU allocation and logistics coordination"
            : daysOut > 30
              ? daysOut +
                " days out — expedited fulfillment needed; focus on hero product lines"
              : daysOut +
                " days out — critical timeline; execute on in-stock inventory only",
    });
    ...
    factors.push({
      ...
      rationale:
        mktScore >= 85
          ? ev.sport +
            " is a priority Nike growth category — aligns with global performance and culture narrative"
          : mktScore >= 70
            ? ev.sport +
              " is a strategic adjacency with lifestyle crossover and youth audience appeal"
            : ev.sport +
              " offers specialist community depth and authentic brand affinity opportunities",
    });
  ...
  function openPopup(idx) {
    ...
    if (ev.athletes && ev.athletes.length) {
      athHtml +=
        '<hr class="p-divider"><div class="p-ath-hdr">DESIGNATED NIKE ATHLETES</div>';
      ev.athletes.forEach((name) => {
        ...
        const detail = [a && a.sport, a && a.nationality]
          .filter(Boolean)
          .join(" · ");
        athHtml +=
          '<div class="ath-row">' +
    ...
    let factorHtml =
      '<hr class="p-divider"><div class="p-ath-hdr">TOP 3 NIKE OPPORTUNITY FACTORS</div>';
    top3.forEach(function (f, i) {
      factorHtml +=
        ...
        (i + 1) +
        "</span>" +
        '<span class="p-f-label">' +
        esc(f.label) +
        "</span>" +
        '<span class="p-f-score">' +
        f.score +
        "</span>" +
        "</div>" +
        '<div class="p-f-bar-bg">' +
        '<div class="p-f-bar" style="width:' +
        f.score +
        "%;background:" +
        col +
        '"></div>' +
        "</div>" +
        '<div class="p-f-rationale">' +
        esc(f.rationale) +
        "</div>" +
        "</div>";
    });
    ...
    document.getElementById("popupBody").innerHTML =
      '<div class="p-sport" style="color:' +
      col +
      '">' +
      esc(ev.sport.toUpperCase()) +
      "</div>" +
      '<div class="p-title">' +
      esc(ev.title) +
      "</div>" +
      ...
      '<div class="p-meta"><span class="p-icon">&#128101;</span><span>' +
      ev.attendance.toLocaleString() +
      " est. attendance</span></div>" +
      (ev.description
        ? '<div class="p-desc">' + esc(ev.description) + "</div>"
        : "") +
      athHtml +
      factorHtml;
